"""Utilities and tools for tracking static with Weights & Biases."""
...
def check_wandb_resume(opt):
    ...
    if isinstance(opt.resume, str):
        if opt.resume.startswith(WANDB_ARTIFACT_PREFIX):
            if RANK not in [-1, 0]:  # For resuming DDP static
...
class WandbLogger():
    """Log training static, datasets, models, and predictions to Weights & Biases.
       ...
       """
        ...
        if isinstance(opt.resume, str):  # checks resume from artifact
            if opt.resume.startswith(WANDB_ARTIFACT_PREFIX):
                ...
                assert wandb, 'install wandb to resume wandb static'
                # Resume wandb-artifact:// static here| workaround for not overwriting wandb.config
            ...
            self.wandb_run = wandb.init(config=opt,
                                        resume="allow",
                                        project='YOLOv5' if opt.project == 'static/train' else Path(opt.project).stem,
                                        entity=opt.entity,
    ...
    def check_and_upload_dataset(self, opt):
        ...
        config_path = self.log_dataset_artifact(opt.data,
                                                opt.single_cls,
                                                'YOLOv5' if opt.project == 'static/train' else Path(opt.project).stem)
    ...
    def download_model_artifact(self, opt):
        ...
        if opt.resume.startswith(WANDB_ARTIFACT_PREFIX):
            ...
            assert not is_finished, 'training is finished, can only resume incomplete static.'
